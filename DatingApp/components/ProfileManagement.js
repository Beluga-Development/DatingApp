  ...
  const createProfile = async () => {
    try {
      ...
      if (result.length > 0) {
        ...
        if (profile.linkedin)
          await api.data.addContact("linkedin", profile.linkedin);
  ...
  useEffect(() => {
    if (props.editMode) {
          ...
          if (data !== null && data !== undefined) {
            ...
            setProfile({
              ...
              desiredSkills: desiredData.map((i) => ({
                id: i.interest_id,
                name:
                  allInterests.find((d) => d.id === i.interest_id)?.name ||
                  "Unknown",
              })),
      ...
      api.data
        ...
        .then((data) => {
          setSkills(data);
          setDesiredSkills(data);
        })
        .catch((err) => console.error("Failed to load desiredSkills:", err));
    }
      ...
      >
          ...
          >
            ...
            >
              ...
              <TouchableOpacity onPress={() => setShowPfpModal(false)}>
                <Text style={{ color: palette.contrast, fontSize: 14 }}>
                  Cancel
                </Text>
              </TouchableOpacity>
            </View>
        ...
        >
          <View style={{ flexDirection: "column", marginHorizontal: 10 }}>
            <View
              id={"ProfilePicture"}
              style={{ marginBottom: 50, marginTop: 15, alignItems: "center" }}
            >
              <TouchableOpacity
            ...
            <View id={"SkillsView"}>
              <ListChipper
                ...
                onRemove={(item) =>
                  setProfile((prev) => ({
                    ...prev,
                    skills: prev.skills.filter((i) => i.id !== item.id),
                  }))
                }
              />
            </View>
            <View id={"InterestView"}>
              <ListChipper
                ...
                onAdd={(item) =>
                  setProfile((prev) => ({
                    ...prev,
                    desiredSkills: [...prev.desiredSkills, item],
                  }))
                }
                onRemove={(item) =>
                  setProfile((prev) => ({
                    ...prev,
                    desiredSkills: prev.desiredSkills.filter(
                      (i) => i.id !== item.id,
                    ),
                  }))
                }
            ...
            <Button
              text={props.editMode ? "Save Changes" : "Create Profile"}
